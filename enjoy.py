...
class AppState:
    def __init__(
        self, skel_state: SkeletonState, skel_actors, axes_actors, zexpms: torch.Tensor
    ):
...
def setup_and_run_gui(pl: ImguiPlotter, app_state: AppState):
    ...
    def gui():
        ...
        local_rotation = torch.as_tensor(expm_to_quat(zexpm[1:].reshape(-1, 3)))
...
def main():
    ...
    # Run the GUI
    app_state = AppState(sk_state, sk_actors, ax_actors, zexpms)
